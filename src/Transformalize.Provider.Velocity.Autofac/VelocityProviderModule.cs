...
namespace Transformalize.Providers.Velocity.Autofac {
   public class VelocityProviderModule : Module {
      ...
      protected override void Load(ContainerBuilder builder) {
         ...
         foreach (var t in process.Templates.Where(t => t.Enabled && t.Engine == Velocity)) {
            var template = t;
         ...
         foreach (var connection in process.Connections.Where(c => c.Provider == Velocity)) {
            builder.Register<ISchemaReader>(ctx => new NullSchemaReader()).Named<ISchemaReader>(connection.Key);
         }

         // Entity input
         foreach (var entity in process.Entities.Where(e => process.Connections.First(c => c.Name == e.Connection).Provider == Velocity)) {
         ...
         if (process.Output().Provider == Velocity) {
            ...
            foreach (var entity in process.Entities) {
               ...
               // ENTITY WRITER
               builder.Register<IWrite>(ctx => new VelocityWriter(ctx.ResolveNamed<OutputContext>(entity.Key), new DefaultReader(new FileReader(), new WebReader()))).Named<IWrite>(entity.Key);
            }
         }
